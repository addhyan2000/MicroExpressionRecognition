...
class AblationOrchestrator:
    ...
    def __init__(self, exp: ExperimentConfig, max_samples: Optional[int] = None) -> None:
        self.exp = exp
        ...
        self._log.info("  device=%s | classes=%s | dataset_filter=%s | expr_filter=%s",
                       self.device, exp.class_names, exp.dataset_filter, exp.expression_filter)
    ...
    def run_matrix(self, only: Optional[str] = None, phase: Optional[str] = None) -> None:
...
def main() -> None:
    ...
    set_seed(exp.seed)
    orchestrator = AblationOrchestrator(exp, max_samples=args.max_samples)
    orchestrator.run_matrix(only=args.only, phase=args.phase)
